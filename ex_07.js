...
MongoClient.connect("mongodb://localhost:27042", {useNewUrlParser: true, useUnifiedTopology: true},function (err, client ) {
    ...
    router.get('/', function (req, res) {
        res.render('vista5/index', {message : '', students : ''});
    })
    ...
    router.get('/admin', function(req, res) {
        ...
        collection.find(query).sort(mysort).toArray(function(err, result) {
            if (err) {
                console.log(err) ;
                res.render('vista7/admin', {students: [], message : 'no student', sort : {sortBy, inOrder}, query:{admin, validated, sortId, id, searchFirstname, searchLastname, searchEmail}  })

                return
            }
            res.render('vista7/admin', {students : result, message : '', sort: {sortBy, inOrder}, query:{admin, validated, sortId, id, searchFirstname, searchLastname, searchEmail} })
        });
    })

    router.get('/admin/add', function(req, res) {
        res.render('vista7/addstudiant', {students : '', message : 'Ajouter'})
    })
    ...
    router.post('/admin/add', async function (req, res) {
        ...
        collection.insertOne(student, function(err, insert) {
            if (err) {
                res.render('vista7/addstudiant', {message : 'Failed to save the collection.'})
                return
            }
    ...
    router.get('/admin/edit/:id', async function(req, res){
        ...
        res.render('vista7/studiantsedit', {message : '', student : user})
    })
